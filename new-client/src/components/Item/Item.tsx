    ...
    useEffect(() => {
        (async () => {
            if(item.imageIds.length > 0) {
                setPreview(item.imageIds[0].includes('http') ? item.imageIds[0] : `http://localhost:3001/api/images/assets/${item.imageIds[0]}`);
            }
        })();
    }, [item])
    return <Link href={`/items/${item.code}`}>
        <a className={styles.wrapper}>
            <div className={styles.item}>
                {preview != '' && <img src={preview} width={270} height={270} alt={item.title} />}
                ...
                <div className={styles.field}>
                    ...
                    <div className={styles.field__value}>{item.platforms.map(platform => {
                        return <Image src={getPlatformIcon(platform)} width={20} height={20} alt={platform} key={platform} />
                    })}</div>
                </div>
                <div className={styles.field}>
                ...
                <div className={styles.field}>
                    ...
                    <div className={styles.field__value}>{item.reviews.length || 0}</div>
                </div>
            </div>
        </a>
    </Link>
